...
--nhập từ bàn phím.
create function fn_demsp(@x money, @y money, @z nvarchar(20))
returns int
as
begin
	declare @dem int
	select @dem = count(masp) from sanpham
	inner join hangsx on sanpham.mahangsx = hangsx.mahangsx
	where giaban >= @x and giaban <= @y and tenhang = @z
	return @dem
end
select dbo.fn_demsp(100000, 20000000, 'OPPO')
